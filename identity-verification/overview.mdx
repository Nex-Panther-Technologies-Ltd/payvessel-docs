...
description: "PayVessel identity verification API for BVN verification, NIN verification, credit score API, blacklist checks, and KYC in Nigeria."
keywords:
  [
...
<CardGroup cols={2}>
  ...
  <Card title="Sandbox Testing" icon="flask" href="/identity-verification/sandbox-testing">
    Reserved test values for simulating verified, not-found, and error responses.
  </Card>
</CardGroup>

<Note>
Every identity endpoint is **simulated in the sandbox** — no request reaches a real bureau. See [Sandbox Testing](/identity-verification/sandbox-testing) for the reserved values that select each response.
</Note>
