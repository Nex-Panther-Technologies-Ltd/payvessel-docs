...
<Note>
Authentication matches other PayVessel APIs. See [Authentication](/api-basics/authentication) for `api-key` and `api-secret` headers.
</Note>
...
The PayVessel API requires **full Nigerian KYC on every create request**. `customer_id` is not accepted — existing customers are matched by **email**.
