...
Set `is_contactless` to `true` to issue a contactless-enabled card (tap-to-pay). Defaults to `false` for a standard virtual card. Contactless cards may have a different issuance fee — use [Card Fee Quote](/virtual-cards/fee-quote) before create.
